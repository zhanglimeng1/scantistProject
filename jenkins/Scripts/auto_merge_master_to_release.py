...
def auto_merge(id):
    ...
    try:
        ...
        mr = project.mergerequests.create({'source_branch': 'master','target_branch': curr_release,'title': title})
        mr.merge()
        print(groupname + ' ' + projectname + ' Merge branch master to ' + curr_release + ' success!')
    except gitlab.exceptions.GitlabMRClosedError: #gitlab.exceptions.GitlabMRClosedError: 405: 405 Method Not Allowed
        print('master已合并过release，跳过')
        exit(0)
        ...
        print(groupname + ' ' + projectname + ' Merge branch master to ' + curr_release + ' failure!' + str(e))
        exit(1)
def git_diff(id):
    '''
    :param id: 仓库id
    ...
    '''
    project = gl.projects.get(id)
    res = project.repository_compare(curr_release,'master')
    if len(res['diffs']) > 0:
        auto_merge(id)
    else:
        print('master分支没有比'+curr_release+'更新的差异文件，所以无需合并！')
